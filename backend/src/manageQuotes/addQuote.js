...
/**
 * Adds a quote to the user's saved quotes
...
const addQuote = async (req, res) => {
    try {
        ...
        // Add quote and save user
        user.savedQuotes.push(quote);
        await user.save();
        
        return res.status(200).json(
            new APIResponse(200, user, "Quote added successfully")
        );
    } catch (error) {
        console.error('Error adding quote:', error);
        return res.status(500).json(
            new APIError(500, null, "Error adding quote")
        );
    }
};
